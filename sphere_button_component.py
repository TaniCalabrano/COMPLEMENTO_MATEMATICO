...
Todo en un solo components.html():
 - Fondo del iframe = #0f1117 (igual al de la app → sin rectángulo visible)
 - Esfera grande con anillos orbitales giratorios
 - Clic detectado via window.parent.location (query param cm_go=1)
 - Sin ningún botón de texto separado
"""
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .scene:active .sphere { transform: scale(0.93); }
  @keyframes pulse {
  ...
  .label {
    ...
    font-family:Georgia,serif; pointer-events:none; padding:0 10px;
  }
</style>
</head>
<body>
<div class="scene" onclick="go()">
...
def mostrar_boton_esfera():
    """
    Renderiza la esfera. Retorna True cuando el usuario hace clic.
    El fondo del iframe coincide con #0f1117 así no se ve ningún rectángulo.
    """
    _, col, _ = st.columns([1, 1, 1])
    with col:
        components.html(SPHERE_HTML, height=212, scrolling=False)
